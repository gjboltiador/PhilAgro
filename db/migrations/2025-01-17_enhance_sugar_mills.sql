-- Migration: Enhance Sugar Mills Table and Add Sample Data
-- This script adds missing columns to the existing sugar_mills table and inserts sample data
...
ADD COLUMN IF NOT EXISTS created_by_user_id BIGINT UNSIGNED NULL COMMENT 'User who created this record' AFTER compliance_status,
ADD COLUMN IF NOT EXISTS updated_by_user_id BIGINT UNSIGNED NULL COMMENT 'User who last updated this record' AFTER created_by_user_id;

-- 2. Update operating_status enum to include 'under_construction'
ALTER TABLE sugar_mills 
MODIFY COLUMN operating_status ENUM('operational', 'maintenance', 'closed', 'seasonal', 'under_construction') DEFAULT 'operational';

-- 3. Insert comprehensive sample sugar mills data
...
-- 6. Display sample sugar mill data for verification
SELECT 
  plant_code,
  short_name,
  city,
...
SELECT 'Sugar Mills table enhanced and sample data added successfully' as status;
